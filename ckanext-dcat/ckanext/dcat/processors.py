...
def serialize_to_digstcsv(dataset_dict):
   csv_lines = []

  # log.info('dataset_dict: ' + '\n'.join(map(str,dataset_dict['extras'])) )
           ...
           for extraField in dataset_dict['extras']:
               if extraField['key'] == field:
                   if type(extraField['value']) is list:
                       data[field] = ', '.join(extraField['key'])
                   else:
                       if extraField['value'][0] == '[':
                           data[field] = extraField['value'][1:-1]
                       else:
                           data[field] = extraField['value']

   line = ';'.join(fields)
   csv_lines.append(line)

   dataFields = []
   for dataField in package_show_extras.split(','):
   ...
   csv_lines.append(';'.join(dataFields))

   csvText = u"\n".join(csv_lines).replace(u"\u2018", "'").replace(u"\u2019", "'").replace(u"\u2013", "-") \
       .replace(u"\u201c", "\"").replace(u"\u201d", "\"").replace(u"\u201e", "\"").replace(u"\u201f", "\"") \
       .encode('iso-8859-1')

   log.info('char encoding: ' + chardet.detect( csvText )['encoding'])
...
class RDFSerializer(RDFProcessor):
    ...
    def serialize_dataset(self, dataset_dict, _format='xml'):
        ...
        if _format == 'json-ld':
            ...
            output = serialize_to_digstcsv(dataset_dict)
        else:
